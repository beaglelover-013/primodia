function cleanPlaceToken(value: string) {
  let cleaned = value
    ...
    .trim();
  cleaned = cleaned
    .replace(/^(?:我|玩家|主角|克斯)?(?:已经|正在|刚刚)?(?:去了|去到|去往|前往|来到|到了|回了|回到|返回到|返回|进入|抵达|离开到|离开|走到|走进|去|回)\s*/g, '')
    .replace(/^了(?=酒馆|厨房|库房|主厅|客房|街坊|市集|市场|商铺|店铺|农田|酒窖|地图)/g, '')
    .trim();
  return cleaned;
}
...
export function isShopLikePlace(place: string) {
  ...
  return /街坊商铺|商铺|店铺|摊位|流动摊|货车|商队|商会|货架/.test(text);
}
...
export function inferSceneType(placeText: string): SceneType {
  ...
  if (/街坊|市集|市场|街口|商铺带/.test(text)) return '街坊';
  ...
  if (/地图|城|镇|村|港|堡|关|谷|林|原|庭|渡/.test(text)) return '地图';
